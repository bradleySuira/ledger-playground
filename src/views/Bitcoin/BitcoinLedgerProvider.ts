...
export class BitcoinLedgerProvider extends BitcoinWalletProvider(LedgerProvider as WalletProviderConstructor) {
  ...
  async _buildTransaction(
    ...
    debugger
    const outputScriptHex = (await (app as any).serializeTransactionOutputs({ outputs })).toString('hex')
  ...
  async signPSBT(data: string, inputs: bitcoin.PsbtInputTarget[]) {
    ...
    if (inputsArePubkey) {
      ...
      const outputScriptHex = (app as any)
